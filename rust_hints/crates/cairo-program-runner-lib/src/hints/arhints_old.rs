...
pub const POSEIDON_HASH_FUNCTION_CHOICE: Felt252 = Felt252::new_const(0);
pub const KECCAK_HASH_FUNCTION_CHOICE: Felt252 = Felt252::new_const(1);
...
pub fn prepare_verifier_tasks_and_bootloader_output_segment(
    vm: &mut VirtualMachine,
    exec_scopes: &mut ExecutionScopes,
    ids_data: &HashMap<String, HintReference>,
    ap_tracking: &ApTracking,
    ...
    // Save aggregator fact topologies and reset current fact topologies.
    ...
    .map(|child_proof| {
        ...
        TaskSpec {
            task: Task::Program(ProgramWithInput {
                program: stark_verifier_program.clone(), // Loaded from file or already pre-parsed
