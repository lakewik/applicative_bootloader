...
func commutative_keccak256{
    ...
    if (is_lt == 1) {
        ...
        let (hash: Uint256) = getKeccakOnlyUint{
            pedersen_ptr=pedersen_ptr, range_check_ptr=range_check_ptr, bitwise_ptr=bitwise_ptr
        }(ordered_a, ordered_b);
        return (hash,);
    } else {
        let ordered_a = b;
...
func getKeccakOnlyUint{pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    a_uint256: Uint256, b_uint256: Uint256
) -> (hash: Uint256) {
    alloc_locals;
    let (local keccak_ptr: felt*) = alloc();
    let keccak_ptr_start = keccak_ptr;
