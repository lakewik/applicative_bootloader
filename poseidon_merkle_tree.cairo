...
func build_next_level{poseidon_ptr: PoseidonBuiltin*, range_check_ptr}(
    data_len : felt, data : felt*, index : felt, next_level : felt*, next_level_len : felt, commutative: felt
) -> (next_level_len : felt) {
    alloc_locals;
    if (index == data_len) {
    ...
    if (doubled == data_len) {
        ...
        return build_next_level(data_len, data, index + 1, next_level, next_level_len + 1, commutative);
    } else {
        // Pair elements and hash
        if (commutative == 1) {
            let (hash) = commutative_poseidon{poseidon_ptr=poseidon_ptr}([data + index], [data + index + 1]);
        } else {
            let (hash) = non_commutative_poseidon{poseidon_ptr=poseidon_ptr}([data + index], [data + index + 1]);
        }
        assert [next_level + next_level_len] = hash;
        return build_next_level(data_len, data, index + 2, next_level, next_level_len + 1, commutative);
    }
}
