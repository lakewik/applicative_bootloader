...
from starkware.cairo.common.uint256 import uint256_lt, felt_to_uint256

// Core functions for Keccak Merkle Tree with carrying over odd nodes //

func hash_leaves_keccak{range_check_ptr, bitwise_ptr: BitwiseBuiltin*, keccak_ptr: KeccakBuiltin*}(
    ...
    //     n_elements=1, elements=hash_input
    // );

    let (hash) = keccak_uint256s{
    ...
    return hash_leaves_keccak(data_len, data, index + 1, hashed_data, hashed_data_len + 1);
}

func build_next_level_keccak{
   range_check_ptr, bitwise_ptr: BitwiseBuiltin*, keccak_ptr: KeccakBuiltin*
    ...
    if (doubled == data_len) {
        ...
        return build_next_level_keccak(
...
func merkle_tree_keccak{ range_check_ptr, bitwise_ptr: BitwiseBuiltin*, keccak_ptr: KeccakBuiltin*}(
...
func non_commutative_keccak256{
    ...
    // Compute the hash
    let (hash) = keccak_uint256s{range_check_ptr=range_check_ptr, bitwise_ptr=bitwise_ptr}(
        n_elements=2, elements=data_uint
    );
...
// Computes the commutative Keccak256 hash of two Uint256 values using keccak256 builtin.
func commutative_keccak256_builtin{
    range_check_ptr, bitwise_ptr: BitwiseBuiltin*, keccak_ptr: KeccakBuiltin*
